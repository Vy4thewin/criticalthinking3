...
col<-training_data%>%select(PARENT1,MSTATUS,SEX,EDUCATION,JOB,CAR_USE,CAR_TYPE,RED_CAR,REVOKED,URBANICITY )
lapply(col, unique)
```
...
```{r}
#Removing index 
train.clean<-training_data%>%select(-(INDEX))
...
train.clean<-train.clean%>%select(-c(EDUCATION,CAR_TYPE,JOB))
```
...
lamb.INCOME<-lamb.INCOME$x[which.max(lamb.INCOME$y)]#
lamb.HOME_VAL<-lamb.HOME_VAL$x[which.max(lamb.HOME_VAL$y)]#
lamb.BLUEBOOK<-lamb.BLUEBOOK$x[which.max(lamb.BLUEBOOK$y)]#
lamb.OLDCLAIM<-lamb.OLDCLAIM$x[which.max(lamb.OLDCLAIM$y)]#
lamb.KIDSDRIV<-lamb.KIDSDRIV$x[which.max(lamb.KIDSDRIV$y)]#
lamb.AGE<-lamb.AGE$x[which.max(lamb.AGE$y)]
lamb.HOMEKIDS<-lamb.HOMEKIDS$x[which.max(lamb.HOMEKIDS$y)]
lamb.YOJ<-lamb.YOJ$x[which.max(lamb.YOJ$y)]
lamb.TRAVTIME<-lamb.TRAVTIME$x[which.max(lamb.TRAVTIME$y)]
lamb.TIF<-lamb.TIF$x[which.max(lamb.TIF$y)]
lamb.MVR_PTS<-lamb.MVR_PTS$x[which.max(lamb.MVR_PTS$y)]
lamb.CLM_FREQ<-lamb.CLM_FREQ$x[which.max(lamb.CLM_FREQ$y)]
lamb.CAR_AGE<-lamb.CAR_AGE$x[which.max(lamb.CAR_AGE$y)]
