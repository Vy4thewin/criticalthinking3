...
#Way 1| to handle NAs, Drop predictor HBP from the data set as missing and use MICE for imputation
...
The coefficients for the two statistical signficant variables - *TEAM_FIELDING_E* and *TEAM_FIELDING_DP* - are negative. This means for every increase in this stat, the lower the less wins. This seems accurate for the variable *TEAM_FIELDING_E* because the more error a team makes, the more chances it gives the other team to score or take a base. I'm not sure why *TEAM_FIELDING_DP* would have a negative coefficient since the double play should be advantageous to a team and not the oponent. Overall this model, has the most predictive values and has the least statistically significant variable.
...
fit2 <- lm(TARGET_WINS ~ TEAM_BATTING_H + TEAM_BATTING_2B + TEAM_BATTING_3B + TEAM_BATTING_HR + TEAM_BATTING_BB + TEAM_BATTING_SO + TEAM_BATTING_HBP, data = train_set)
summary(fit2)
```

All the statistical significant variables with the exception of TEAM_BATTING_SO has a positive coefficient. This is accruate because *TEAM_BATTING_H*, *TEAM_BATTING_HR*, and *TEAM_BATTING_BB* are all indication of the steal of a base, or a score against the oppossing team. It makes sense that that the higher these variables are, the higher chances for a win. In contrast, TEAM_BATTING_SO 0 which measures the strike outs by a team - indicates the lack of scoring and would therefore make sense for it to negatively affect winning. Overall this has less predictor values compared to model1 and also contains more significant values.
...
In comparison to the other models this has the most significant variables and is more parsimonious. *TEAM_PITCHING_H*, *TEAM_PITCHING_HR*, *TEAM_PITCHING_BB*, and *TEAM_PITCHING_SO* are all positive coefficients because they are crucial to a team's defense. The higher these statistics, the higher the chance for a team to win. 
