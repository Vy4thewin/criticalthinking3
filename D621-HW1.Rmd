...
When visualizing correlation strength between target wins and the other predictor variables by using ggpairs, we can see that there aren't many significant relationships. The batting variables all have positive correlations with Target Wins. Some of the pitching and fielding varialbes have negative correlations. A trend we're seeing is that the offensive variables, which include batting, lead to more wins while some of the defensive stats can negatively affect wins. When creating our models, it may be worth creating them with this in mind. Some of the standout pairings are listed below 
...
see the effects of all the stats against *TARGET_WINS*.
...
In looking at the residual plot models, we can observe linearity and homoscedasticity (variance is constant). We can also see that most points are close to the straight line, which tells us the residuals are normally distributed. In the final visual, we can notice a few deviated points in the model.
...
Though this model meets linearity and homoscedasticity, the first model appears more linear. In the QQ plot, the residual points mostly fall on the line, with the end and front of the tail deviating slightly from it. We can observe a few outlier points. 
