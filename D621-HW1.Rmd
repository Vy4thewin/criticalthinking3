...
The first technique consists of completely removing all data points containing any amount of NAs. In other words, this method filters to rows that are complete. This method includes all the original predictors at the risk of removing more than 90% of the dataset. A reduction from 2276 to 191 data points. This technique is not ideal as the reduced number of observations can negatively influence the model's regression. However, a model with this technique will be explored. 

The alternative path consist utilizes imputation to assign values. Imputation can take many different forms, but they all accomplish the same goal: to assume a value based on the distribution of the data. For this particular analysis, MICE is used to predict the
...
### 2.1 Technique #1: Removing Rows With Any NAs
```{r}
#Technique #1: removing rows with any NAs
training_set_na_rem <- na.omit(training_set)

summary(training_set_na_rem)
```
  
### 2.2 Technique #2: Drop HBP and CS predictors, and use MICE imputation
...
Although our observations were significantly reduced, it did open the
